...
_install_dependencies() {
  ...
  if [ "$_distro" = "Debian" -o "$_distro" = "Ubuntu" ]; then
    ...
    msg2 "Installing dependencies"
    sudo zypper install -y hostname bc bison ccache dwarves elfutils flex gcc-c++ git libXi-devel libelf-devel libqt5-qtbase-common-devel libqt5-qtbase-devel lz4 make ncurses-devel openssl-devel patch pesign rpm-build rpmdevtools schedtool python3 rsync zstd libdw-devel update-bootloader-rpm-macros update-bootloader kernel-install-tools ${clang_deps}
  fi
}
