...
- Editing the [customization.cfg](./customization.cfg) file
- Define the variables from the file above in the environment
- Simply following the interactive install script (it does _not_ prompt for all of them).

To pull updates easily, an external config file can be used instead of the shipped `customization.cfg` file. The default path for the the file is `$HOME/.config/frogminer/linux-tkg.cfg`, but it can be changed with `_EXT_CONFIG_PATH` variable, either in the environment or in `customization.cfg`.
